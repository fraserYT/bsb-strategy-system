...
new_modules = [
    ...
    # 60 — create Tier 1 if missing
    drive_mod(60, "Create Tier 1 (company) folder",
        "[{{58.tla}}] {{58.client_name}}",
        CLIENT_PROJECTS,
        flt("Only if Tier 1 folder missing", empty("{{58.tier1_folder_id}}")),
        x=5700),

    # 61 — create Tier 2 if missing
    drive_mod(61, "Create Tier 2 (contact) folder",
        "[{{2.`5`}}] {{58.primary_contact}}",
        TIER1_ID,
        flt("Only if Tier 2 folder missing", empty("{{58.tier2_folder_id}}")),
        x=6000),

    # 62 — store Tier 1 + 2 IDs if either was just created
    pg_mod(62, "update_client_folder_ids", "Store Tier 1+2 folder IDs",
        {
            "@01:text": "{{2.`5`}}",
            "@02:text": TIER1_ID,
            "@03:text": TIER2_ID
    ...
    # 63 — create Tier 3 if missing
    drive_mod(63, "Create Tier 3 (product type) folder",
        "[{{2.`5`}}] {{57.tier3FolderName}}",
        TIER2_ID,
        flt("Only if Tier 3 folder missing", empty("{{59.product_type_folder_id}}")),
        x=6600),

    # 64 — create Tier 4 if missing
    drive_mod(64, "Create Tier 4 (year) folder",
        YEAR_EXPR,
        TIER3_ID,
        flt("Only if Tier 4 folder missing", empty("{{59.year_folder_id}}")),
        x=6900),

    # 65 — store Tier 3 + 4 IDs if either was just created
    pg_mod(65, "upsert_product_folder", "Store Tier 3+4 folder IDs",
        {
            "@01:text": "{{2.`5`}}",
            "@02:text": "{{40.`0`}}",
            "@03:text": YEAR_EXPR,
